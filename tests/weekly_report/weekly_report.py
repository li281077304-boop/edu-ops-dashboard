...
def parse_wps(path: Path, week: int) -> dict[str, Any]:
    ...
    return {"workbook_type": kind, "sheet": sheet_name, "teachers": teachers, "students": totals, "changes": changes, "single_subject_total": single_subject_total}
...
def parse_tms(path: Path, week: int) -> dict[str, Any]:
    ...
    return {"workbook_type": kind, "teachers": dict(by_teacher), "totals": totals}
...
def build_snapshot(wps_path: Path, tms_path: Path, golden_path: Path, week: int) -> dict[str, Any]:
    ...
    for name, value in wps["teachers"].items():
        ...
        teachers.append({"name": name, "status": "historical_or_active", "one_to_one": value["one_to_one"], "class": value["class"], "subject_count": value["subject_count"], "hours": value["hours"], "sessions": value["sessions"], "leave": value["leave"], "extra": value["extra"], "expansion": value["expansion"], "production": tms_value})
    warnings = ["bkh physical unit and conversion are not confirmed; dependent average metrics remain unresolved.", "refund source is outside the P0 weekly-report scope.", "WPS contains historical/special teachers; active-roster inclusion is not inferred from names alone."]
    unresolved = [{"metric": "students.changes.refund", "status": "SOURCE_MISSING", "reason": "weekly workflow does not have a verified refund source"}, {"metric": "production.average_hours_and_sessions", "status": "BUSINESS_RULE_MISSING", "reason": "bkh unit/teacher denominator not confirmed"}, {"metric": "teachers.active_roster", "status": "BUSINESS_RULE_MISSING", "reason": "teacher_config distinguishes active/special teachers but the weekly source does not carry a signed roster snapshot"}]
    return {
        "version": "1.0",
        ...
        "students": {**students, "single_subject_total": wps["single_subject_total"]},
        ...
        "production": {"tms": tms["totals"], "golden": golden["production"]},
        "teachers": teachers,
        "manual_adjustments": [],
        "source_metadata": [{"path": str(p), "sha256": _sha256(p), "role": role} for p, role in ((wps_path, "WPS"), (tms_path, "TMS"), (golden_path, "FINAL_GOLDEN"))],
        "warnings": warnings,
...
def main() -> None:
    ...
    snapshot = build_snapshot(args.wps, args.tms, args.golden, args.week)

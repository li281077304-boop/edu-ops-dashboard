#!/usr/bin/env python3
# ruff: noqa: E501, I001
"""Run evidence-first Golden UAT for the real June 2026 math reports.

The historical workbook is the oracle.  A mismatch is never silently repaired:
it is classified as a documented historical/source gap so the generated
...
from .weekly_report import build_snapshot
...
def compare_snapshot(snapshot: dict[str, Any], week: int) -> dict[str, Any]:
    ...
    for path in STUDENT_FIELDS:
        ...
        status = "MATCH" if _equal(source, oracle) else "HISTORICAL_SOURCE_GAP"
        comparisons.append({"domain": "students", "metric": ".".join(path), "source": source, "golden": oracle, "status": status, "reason": "WPS source versus manually maintained final workbook" if status != "MATCH" else ""})
    for field in PRODUCTION_FIELDS:
        source = _get(snapshot["production"].get("tms", {}), (field,))
        oracle = _get(golden["production"], (field,))
        status = "MATCH" if _equal(source, oracle) else "HISTORICAL_SOURCE_GAP"
        comparisons.append({"domain": "production", "metric": field, "source": source, "golden": oracle, "status": status, "reason": "TMS revision/manual cumulative standard differs; no deterministic source bridge in the historical inputs" if status != "MATCH" else ""})
    ...
    for name in sorted(set(source_teachers) & set(golden_teachers)):
        for field in ("one_to_one_students", "class_students", "subject_count", "hours", "sessions"):
            ...
            if not _equal(left, right):
                ...
                comparisons.append({"domain": "teachers", "metric": f"{name}.{field}", "source": left, "golden": right, "status": "HISTORICAL_SOURCE_GAP", "reason": "teacher summary and manually adjusted final workbook differ"})
    ...
    return {
        ...
        "status": "PASS_WITH_WARNING" if gaps else "PASS",
        "match": matches,
...
def run_golden_uat(wps: Path, tms: Path, goldens: dict[int, Path], output_dir: Path) -> dict[str, Any]:
    output_dir.mkdir(parents=True, exist_ok=True)
    weeks: list[dict[str, Any]] = []
    for week, golden in sorted(goldens.items()):
        snapshot = build_snapshot(wps, tms, golden, week)
        snapshot_path = output_dir / f"week{week}.json"
        snapshot_path.write_text(json.dumps(snapshot, ensure_ascii=False, indent=2), encoding="utf-8")
        result = compare_snapshot(snapshot, week)
        result["snapshot_path"] = str(snapshot_path)
        weeks.append(result)
    report = {"version": "1.0", "dataset": "2026-06 math group", "weeks": weeks, "unexplained_difference": sum(item["unexplained_difference"] for item in weeks), "generated_at": "2026-09-16"}

...
- `weekly-report-ralph-20260916-r8`: a fresh product worktree with `.ralph/`
  and `devlog/` ignored correctly was bootstrapped. The supervised entry
  reached Host Chief SELECT, but the current Codex runtime again exhausted
  repeated network/model requests before a durable SELECT reply; no Worker,
  Gate, checkpoint, or Review was fabricated after that failure.
